...
-- Extensiones (PostGIS para búsqueda geográfica)
...
ALTER ROLE core_app_user IN DATABASE core_db SET search_path = core, public;

ALTER DEFAULT PRIVILEGES IN SCHEMA core
GRANT SELECT, INSERT, UPDATE, DELETE ON TABLES TO core_app_user;

ALTER DEFAULT PRIVILEGES IN SCHEMA core
GRANT USAGE, SELECT, UPDATE ON SEQUENCES TO core_app_user;
...
CREATE TABLE IF NOT EXISTS core.customer_partner_links (
  ...
  source        text NULL, -- order|booking|lead
...
CREATE TABLE IF NOT EXISTS core.interactions (
  ...
  type         text NOT NULL, -- order|booking|lead|message|call
...
CREATE TABLE IF NOT EXISTS core.payments (
  ...
  external_reference  text NOT NULL, -- order:{id} | booking:{id} | subscription:{id}
  amount              numeric(14,2) NOT NULL,
  currency            text NOT NULL DEFAULT 'CLP',
  status              text NOT NULL CHECK (status IN ('pending','approved','rejected','cancelled','expired')),
  gateway_intent_id   uuid NULL,     -- referencia al intent del gateway (sin FK cross-db)
  provider_token      text NULL,     -- token_ws u otro
...
-- =========================
-- OPTIONAL: updated_at triggers (si quieres timestamps automáticos)
-- =========================
...
DROP TRIGGER IF EXISTS trg_core_partners_updated_at ON core.partners;
CREATE TRIGGER trg_core_partners_updated_at
BEFORE UPDATE ON core.partners
FOR EACH ROW EXECUTE FUNCTION core.set_updated_at();

DROP TRIGGER IF EXISTS trg_core_customers_updated_at ON core.customers;
CREATE TRIGGER trg_core_customers_updated_at
BEFORE UPDATE ON core.customers
FOR EACH ROW EXECUTE FUNCTION core.set_updated_at();

DROP TRIGGER IF EXISTS trg_core_orders_updated_at ON core.orders;
CREATE TRIGGER trg_core_orders_updated_at
BEFORE UPDATE ON core.orders
FOR EACH ROW EXECUTE FUNCTION core.set_updated_at();

DROP TRIGGER IF EXISTS trg_core_bookings_updated_at ON core.bookings;
CREATE TRIGGER trg_core_bookings_updated_at
BEFORE UPDATE ON core.bookings
FOR EACH ROW EXECUTE FUNCTION core.set_updated_at();

DROP TRIGGER IF EXISTS trg_core_payments_updated_at ON core.payments;
CREATE TRIGGER trg_core_payments_updated_at
